...
int32_T sm_ssci_logFcn_codeGen(const NeDae *dae,
  ...
  PmRealVector *output);
PmfMessageId servo_mixed_test_e8940bbe_1_deriv(
  const double *,
...
PmfMessageId servo_mixed_test_e8940bbe_1_checkDynamics(
  const double *,
  const double *, const double *, const double *,
  const double *,
  double *,
  NeuDiagnosticManager *neDiagMgr);
PmfMessageId servo_mixed_test_e8940bbe_1_outputDyn(
  const double *,
  const double *, const double *, const double *,
  const double *,
  double *,
  ...
  double *,
  NeuDiagnosticManager *neDiagMgr);
PmfMessageId servo_mixed_test_e8940bbe_1_outputKin(
  const double *,
  const double *, const double *, const double *,
  const double *,
  double *,
  NeuDiagnosticManager *neDiagMgr);
PmfMessageId servo_mixed_test_e8940bbe_1_output (
  const double *,
  const double *, const double *, const double *,
  const double *,
  double *,
  NeuDiagnosticManager *neDiagMgr);
void servo_mixed_test_e8940bbe_1_resetStateVector(const void *mech, double
  *stateVector);
void servo_mixed_test_e8940bbe_1_initializeTrackedAngleState(
  const void *mech,
  const double *motionData,
  double *stateVector,
  void *neDiagMgr);
void servo_mixed_test_e8940bbe_1_computeDiscreteState(const void *mech, double
  *stateVector);
void servo_mixed_test_e8940bbe_1_adjustPosition(
  const void *mech,
  const double *dofDeltas,
  double *stateVector);
void servo_mixed_test_e8940bbe_1_perturbState(
  const void *mech,
  size_t stageIdx,
  size_t primitiveIdx,
  double magnitude,
  ...
  double *stateVector);
void servo_mixed_test_e8940bbe_1_computeDofBlendMatrix(
  const void *mech,
  size_t stageIdx,
  size_t primitiveIdx,
  ...
  int partialType,
  double *matrix);
void servo_mixed_test_e8940bbe_1_projectPartiallyTargetedPos(
  const void *mech,
  size_t stageIdx,
  size_t primitiveIdx,
  ...
  int partialType,
  double *stateVector);
void servo_mixed_test_e8940bbe_1_propagateMotion(
  const void *mech,
  const double *stateVector,
  double *motionData);
size_t servo_mixed_test_e8940bbe_1_computeAssemblyError(
  const void *mech,
  size_t constraintIdx,
  const double *stateVector,
  const double *motionData,
  double *error);
size_t servo_mixed_test_e8940bbe_1_computeAssemblyJacobian(
  const void *mech,
  size_t constraintIdx,
  ...
  const double *motionData,
  double *J);
size_t servo_mixed_test_e8940bbe_1_computeFullAssemblyJacobian(
  const void *mech,
  const double *stateVector,
  const double *motionData,
  double *J);
int servo_mixed_test_e8940bbe_1_isInKinematicSingularity(
  const void *mech,
  size_t constraintIdx,
  const double *motionData);
PmfMessageId servo_mixed_test_e8940bbe_1_convertStateVector(
  const void *asmMech,
  const void *simMech,
  const double *asmStateVector,
  double *simStateVector,
  void *neDiagMgr);
void servo_mixed_test_e8940bbe_1_constructStateVector(
  const void *mech,
...
void servo_mixed_test_e8940bbe_1_extractSolverStateVector(
  const void *mech,
  const double *fullStateVector,
  double *solverStateVector);
int servo_mixed_test_e8940bbe_1_isPositionViolation(
  const void *mech,
  const double *stateVector);
int servo_mixed_test_e8940bbe_1_isVelocityViolation(
  const void *mech,
  const double *stateVector);
PmfMessageId servo_mixed_test_e8940bbe_1_projectStateSim(
  const void *mech,
  const double *inputVector,
  double *stateVector,
  void *neDiagMgr);
void servo_mixed_test_e8940bbe_1_computeConstraintError(
  const void *mech,
  const double *stateVector,
  double *error);
PmfMessageId servo_mixed_test_e8940bbe_1_assemble(const double *u, double *udot,
  double *x,
  NeuDiagnosticManager *neDiagMgr)
{
  (void) x;
  ...
  NeuDiagnosticManager *neDiagMgr)
{
  ...
  if (smData->mCachedDerivativesAvailable)
    ...
    errorId = servo_mixed_test_e8940bbe_1_deriv(
      systemInput->mX.mX,
      systemInput->mU.mX,
      systemInput->mU.mX + 7,
      systemInput->mV.mX + 7,
  ...
  NeuDiagnosticManager *neDiagMgr)
{
  ...
  if (errorId == NULL && smData->mDoCheckDynamics) {
    double result = 0.0;
    errorId = servo_mixed_test_e8940bbe_1_checkDynamics(
      systemInput->mX.mX,
      systemInput->mU.mX,
      systemInput->mU.mX + 7,
      systemInput->mV.mX + 7,
  ...
  void initIoInfo(NeDaePrivateData *smData)
{
  const char *inputPortPaths[7] = {
    "Subsystem1.Simulation.Revolute_Joint1.qi",
    "Subsystem1.Simulation.Revolute_Joint2.qi",
    "Subsystem1.Simulation.Revolute_Joint3.qi",
    "Subsystem1.Simulation.Revolute_Joint4.qi",
    "Subsystem1.Simulation.Revolute_Joint5.qi",
    "Subsystem1.Simulation.Revolute_Joint6.qi",
    "Subsystem1.Simulation.Revolute_Joint7.qi"
  };
  ...
  void initAssemblyDelegate(SmMechanismDelegate *delegate)
{
  ...
  delegate->mResetStateVector = servo_mixed_test_e8940bbe_1_resetStateVector;
  delegate->mInitializeTrackedAngleState =
    servo_mixed_test_e8940bbe_1_initializeTrackedAngleState;
  delegate->mComputeDiscreteState =
    servo_mixed_test_e8940bbe_1_computeDiscreteState;
  delegate->mAdjustPosition = servo_mixed_test_e8940bbe_1_adjustPosition;
  delegate->mPerturbState = servo_mixed_test_e8940bbe_1_perturbState;
  delegate->mComputeDofBlendMatrix =
    servo_mixed_test_e8940bbe_1_computeDofBlendMatrix;
  delegate->mProjectPartiallyTargetedPos =
    servo_mixed_test_e8940bbe_1_projectPartiallyTargetedPos;
  delegate->mPropagateMotion = servo_mixed_test_e8940bbe_1_propagateMotion;
  delegate->mComputeAssemblyError =
    servo_mixed_test_e8940bbe_1_computeAssemblyError;
  delegate->mComputeAssemblyJacobian =
    servo_mixed_test_e8940bbe_1_computeAssemblyJacobian;
  delegate->mComputeFullAssemblyJacobian =
    servo_mixed_test_e8940bbe_1_computeFullAssemblyJacobian;
  delegate->mIsInKinematicSingularity =
    servo_mixed_test_e8940bbe_1_isInKinematicSingularity;
  delegate->mConvertStateVector = servo_mixed_test_e8940bbe_1_convertStateVector;
  delegate->mConstructStateVector =
    servo_mixed_test_e8940bbe_1_constructStateVector;
  delegate->mExtractSolverStateVector =
    servo_mixed_test_e8940bbe_1_extractSolverStateVector;
  delegate->mIsPositionViolation =
    servo_mixed_test_e8940bbe_1_isPositionViolation;
  delegate->mIsVelocityViolation =
    servo_mixed_test_e8940bbe_1_isVelocityViolation;
  delegate->mProjectStateSim = servo_mixed_test_e8940bbe_1_projectStateSim;
  delegate->mComputeConstraintError =
    servo_mixed_test_e8940bbe_1_computeConstraintError;
  delegate->mMech = NULL;
}
...
void servo_mixed_test_e8940bbe_1_NeDaePrivateData_create(NeDaePrivateData
  *smData)
{
