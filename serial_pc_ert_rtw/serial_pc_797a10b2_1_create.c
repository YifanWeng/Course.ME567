...
int32_T sm_ssci_logFcn_codeGen(const NeDae *dae,
  ...
  PmRealVector *output);
PmfMessageId serial_pc_797a10b2_1_deriv(
  const double *,
...
PmfMessageId serial_pc_797a10b2_1_checkDynamics(
  const double *,
  const double *, const double *, const double *,
  const double *,
  double *,
  NeuDiagnosticManager *neDiagMgr);
PmfMessageId serial_pc_797a10b2_1_outputDyn(
  const double *,
  const double *, const double *, const double *,
  const double *,
  double *,
  ...
  double *,
  NeuDiagnosticManager *neDiagMgr);
PmfMessageId serial_pc_797a10b2_1_outputKin(
  const double *,
  const double *, const double *, const double *,
  const double *,
  double *,
  NeuDiagnosticManager *neDiagMgr);
PmfMessageId serial_pc_797a10b2_1_output (
  const double *,
  const double *, const double *, const double *,
  const double *,
  double *,
  NeuDiagnosticManager *neDiagMgr);
void serial_pc_797a10b2_1_resetStateVector(const void *mech, double *stateVector);
void serial_pc_797a10b2_1_initializeTrackedAngleState(
  const void *mech,
  const double *motionData,
  double *stateVector,
  void *neDiagMgr);
void serial_pc_797a10b2_1_computeDiscreteState(const void *mech, double
  *stateVector);
void serial_pc_797a10b2_1_adjustPosition(
  const void *mech,
  const double *dofDeltas,
  double *stateVector);
void serial_pc_797a10b2_1_perturbState(
  const void *mech,
  size_t stageIdx,
  size_t primitiveIdx,
  double magnitude,
  ...
  double *stateVector);
void serial_pc_797a10b2_1_computeDofBlendMatrix(
  const void *mech,
  size_t stageIdx,
  size_t primitiveIdx,
  ...
  int partialType,
  double *matrix);
void serial_pc_797a10b2_1_projectPartiallyTargetedPos(
  const void *mech,
  size_t stageIdx,
  size_t primitiveIdx,
  ...
  int partialType,
  double *stateVector);
void serial_pc_797a10b2_1_propagateMotion(
  const void *mech,
  const double *stateVector,
  double *motionData);
size_t serial_pc_797a10b2_1_computeAssemblyError(
  const void *mech,
  size_t constraintIdx,
  const double *stateVector,
  const double *motionData,
  double *error);
size_t serial_pc_797a10b2_1_computeAssemblyJacobian(
  const void *mech,
  size_t constraintIdx,
  ...
  const double *motionData,
  double *J);
size_t serial_pc_797a10b2_1_computeFullAssemblyJacobian(
  const void *mech,
  const double *stateVector,
  const double *motionData,
  double *J);
int serial_pc_797a10b2_1_isInKinematicSingularity(
  const void *mech,
  size_t constraintIdx,
  const double *motionData);
PmfMessageId serial_pc_797a10b2_1_convertStateVector(
  const void *asmMech,
  const void *simMech,
  const double *asmStateVector,
  double *simStateVector,
  void *neDiagMgr);
void serial_pc_797a10b2_1_constructStateVector(
  const void *mech,
...
void serial_pc_797a10b2_1_extractSolverStateVector(
  const void *mech,
  const double *fullStateVector,
  double *solverStateVector);
int serial_pc_797a10b2_1_isPositionViolation(
  const void *mech,
  const double *stateVector);
int serial_pc_797a10b2_1_isVelocityViolation(
  const void *mech,
  const double *stateVector);
PmfMessageId serial_pc_797a10b2_1_projectStateSim(
  const void *mech,
  const double *inputVector,
  double *stateVector,
  void *neDiagMgr);
void serial_pc_797a10b2_1_computeConstraintError(
  const void *mech,
  const double *stateVector,
  double *error);
PmfMessageId serial_pc_797a10b2_1_assemble(const double *u, double *udot, double
  *x,
  NeuDiagnosticManager *neDiagMgr)
{
  (void) x;
  ...
  NeuDiagnosticManager *neDiagMgr)
{
  ...
  if (smData->mCachedDerivativesAvailable)
    ...
    errorId = serial_pc_797a10b2_1_deriv(
      systemInput->mX.mX,
      systemInput->mU.mX,
      systemInput->mU.mX + 7,
      systemInput->mV.mX + 7,
  ...
  NeuDiagnosticManager *neDiagMgr)
{
  ...
  if (errorId == NULL && smData->mDoCheckDynamics) {
    double result = 0.0;
    errorId = serial_pc_797a10b2_1_checkDynamics(
      systemInput->mX.mX,
      systemInput->mU.mX,
      systemInput->mU.mX + 7,
      systemInput->mV.mX + 7,
  ...
  void initAssemblyDelegate(SmMechanismDelegate *delegate)
{
  ...
  delegate->mResetStateVector = serial_pc_797a10b2_1_resetStateVector;
  delegate->mInitializeTrackedAngleState =
    serial_pc_797a10b2_1_initializeTrackedAngleState;
  delegate->mComputeDiscreteState = serial_pc_797a10b2_1_computeDiscreteState;
  delegate->mAdjustPosition = serial_pc_797a10b2_1_adjustPosition;
  delegate->mPerturbState = serial_pc_797a10b2_1_perturbState;
  delegate->mComputeDofBlendMatrix = serial_pc_797a10b2_1_computeDofBlendMatrix;
  delegate->mProjectPartiallyTargetedPos =
    serial_pc_797a10b2_1_projectPartiallyTargetedPos;
  delegate->mPropagateMotion = serial_pc_797a10b2_1_propagateMotion;
  delegate->mComputeAssemblyError = serial_pc_797a10b2_1_computeAssemblyError;
  delegate->mComputeAssemblyJacobian =
    serial_pc_797a10b2_1_computeAssemblyJacobian;
  delegate->mComputeFullAssemblyJacobian =
    serial_pc_797a10b2_1_computeFullAssemblyJacobian;
  delegate->mIsInKinematicSingularity =
    serial_pc_797a10b2_1_isInKinematicSingularity;
  delegate->mConvertStateVector = serial_pc_797a10b2_1_convertStateVector;
  delegate->mConstructStateVector = serial_pc_797a10b2_1_constructStateVector;
  delegate->mExtractSolverStateVector =
    serial_pc_797a10b2_1_extractSolverStateVector;
  delegate->mIsPositionViolation = serial_pc_797a10b2_1_isPositionViolation;
  delegate->mIsVelocityViolation = serial_pc_797a10b2_1_isVelocityViolation;
  delegate->mProjectStateSim = serial_pc_797a10b2_1_projectStateSim;
  delegate->mComputeConstraintError =
    serial_pc_797a10b2_1_computeConstraintError;
  delegate->mMech = NULL;
}
...
void serial_pc_797a10b2_1_NeDaePrivateData_create(NeDaePrivateData *smData)
{
